...
var versionRegex = /\d+\.\d+\.\d+[^'"]*/,
    ...
    inAppMessagingPodRegEx =
        /pod 'FirebaseInAppMessaging', '(\d+\.\d+\.\d+[^'"]*)'/,
    standardFirestorePodRegEx =
        /pod 'FirebaseFirestore', '(\d+\.\d+\.\d+[^'"]*)'/,
    googleSignInPodRegEx = /pod 'GoogleSignIn', '(\d+\.\d+\.\d+[^'"]*)'/,
    googleTagManagerPodRegEx =
        /pod 'GoogleTagManager', '(\d+\.\d+\.\d+[^'"]*)'/,
    prebuiltFirestorePodRegEx =
        /pod 'FirebaseFirestore', :tag => '(\d+\.\d+\.\d+[^'"]*)', :git => 'https:\/\/github.com\/invertase\/firestore-ios-sdk-frameworks.git'/,
    prebuiltFirestorePodTemplate =
        "pod 'FirebaseFirestore', :tag => '{version}', :git => 'https://github.com/invertase/firestore-ios-sdk-frameworks.git'",
    // Matches: platform :ios, '14.0'  OR  platform :ios, '12.3.0'
    iosDeploymentTargetPodRegEx = /platform\s*:ios,\s*'(\d+\.\d+(?:\.\d+)?)'/;
...
module.exports = {
    ...
    getXcodeProjectPath: function (context) {
        ...
        return path.join(iosDir, xcodeprojDir, "project.pbxproj");
    },
    ...
    removeShellScriptBuildPhase: function (context, xcodeProjectPath) {
        ...
        for (var buildPhaseId in buildPhases) {
            ...
            if (buildPhaseId.indexOf("_comment") === -1) {
                ...
                shouldDelete =
                    buildPhase.name &&
                    buildPhase.name.indexOf(commentTest) !== -1;
            } else {
    ...
    addGoogleTagManagerContainer: function (context, xcodeProjectPath) {
        ...
        if (utilities.directoryExists(containerDirectorySource)) {
            ...
            try {
                fs.cpSync(containerDirectorySource, containerDirectoryTarget, {
                    recursive: true,
                });
                const appPBXGroup = xcodeProject.findPBXGroupKey({
                    name: appName,
                });
    ...
    ensureRunpathSearchPath: function (context, xcodeProjectPath) {
        function addRunpathSearchBuildProperty(proj, build) {
            let LD_RUNPATH_SEARCH_PATHS = proj.getBuildProperty(
            ...
            LD_RUNPATH_SEARCH_PATHS.forEach((LD_RUNPATH_SEARCH_PATH) => {
                ...
                if (
                    LD_RUNPATH_SEARCH_PATH.indexOf(
                        "@executable_path/Frameworks",
                    ) == -1
                ) {
                    var newValue = LD_RUNPATH_SEARCH_PATH.substr(
                        0,
                        LD_RUNPATH_SEARCH_PATH.length - 1,
                    );
    ...
    applyPodsPostInstall: function (pluginVariables, iosPlatform) {
        ...
        var podFile = fs.readFileSync(podFilePath).toString(),
            DEBUG_INFORMATION_FORMAT =
                pluginVariables["IOS_STRIP_DEBUG"] &&
                pluginVariables["IOS_STRIP_DEBUG"] === "true"
                    ? "dwarf"
                    : "dwarf-with-dsym",
            iosDeploymentTargetMatch = podFile.match(
                iosDeploymentTargetPodRegEx,
            ),
            IPHONEOS_DEPLOYMENT_TARGET = iosDeploymentTargetMatch
                ? iosDeploymentTargetMatch[1]
                : null;
...
post_install do |installer|
    ...
    applyPluginVarsToPlists: function (pluginVariables, iosPlatform) {
        ...
        var entitlementsDebugPlistPath = path.resolve(
            iosPlatform.entitlementsDebugPlist,
        );
        if (!fs.existsSync(entitlementsDebugPlistPath)) {
            utilities.warn(
                `Entitlements debug plist not found at ${entitlementsDebugPlistPath}`,
            );
            return;
        }

        var entitlementsReleasePlistPath = path.resolve(
            iosPlatform.entitlementsReleasePlist,
        );
        if (!fs.existsSync(entitlementsReleasePlistPath)) {
            utilities.warn(
                `Entitlements release plist not found at ${entitlementsReleasePlistPath}`,
            );
            return;
        }
        ...
        var googlePlist = plist.parse(fs.readFileSync(googlePlistPath, "utf8")),
            ...
            entitlementsDebugPlist = plist.parse(
                fs.readFileSync(entitlementsDebugPlistPath, "utf8"),
            ),
            entitlementsReleasePlist = plist.parse(
                fs.readFileSync(entitlementsReleasePlistPath, "utf8"),
            ),
        ...
        if (
            typeof pluginVariables["FIREBASE_ANALYTICS_COLLECTION_ENABLED"] !==
            "undefined"
        ) {
            googlePlist["FIREBASE_ANALYTICS_COLLECTION_ENABLED"] =
                pluginVariables["FIREBASE_ANALYTICS_COLLECTION_ENABLED"] !==
                "false"
                    ? "true"
                    : "false";
        ...
        if (
            typeof pluginVariables[
                "FIREBASE_PERFORMANCE_COLLECTION_ENABLED"
            ] !== "undefined"
        ) {
            googlePlist["FIREBASE_PERFORMANCE_COLLECTION_ENABLED"] =
                pluginVariables["FIREBASE_PERFORMANCE_COLLECTION_ENABLED"] !==
                "false"
                    ? "true"
                    : "false";
            googlePlistModified = true;
        ...
        if (
            ...
            googlePlist["GOOGLE_ANALYTICS_ADID_COLLECTION_ENABLED"] =
                pluginVariables["GOOGLE_ANALYTICS_ADID_COLLECTION_ENABLED"] !==
                "false"
                    ? "true"
                    : "false";
            googlePlistModified = true;
        ...
        if (
            ...
            googlePlist["GOOGLE_ANALYTICS_DEFAULT_ALLOW_ANALYTICS_STORAGE"] =
                pluginVariables[
                    "GOOGLE_ANALYTICS_DEFAULT_ALLOW_ANALYTICS_STORAGE"
                ] !== "false"
                    ? "true"
        ...
        if (
            ...
            googlePlist["GOOGLE_ANALYTICS_DEFAULT_ALLOW_AD_STORAGE"] =
                pluginVariables["GOOGLE_ANALYTICS_DEFAULT_ALLOW_AD_STORAGE"] !==
                "false"
                    ? "true"
                    : "false";
            googlePlistModified = true;
        ...
        if (
            ...
            googlePlist["GOOGLE_ANALYTICS_DEFAULT_ALLOW_AD_USER_DATA"] =
                pluginVariables[
                    "GOOGLE_ANALYTICS_DEFAULT_ALLOW_AD_USER_DATA"
                ] !== "false"
                    ? "true"
        ...
        if (
            typeof pluginVariables["IOS_SHOULD_ESTABLISH_DIRECT_CHANNEL"] !==
            "undefined"
        ) {
            appPlist["shouldEstablishDirectChannel"] =
                pluginVariables["IOS_SHOULD_ESTABLISH_DIRECT_CHANNEL"] ===
                "true";
            appPlistModified = true;
        }
        ...
        if (pluginVariables["IOS_ENABLE_APPLE_SIGNIN"] === "true") {
            entitlementsDebugPlist["com.apple.developer.applesignin"] = [
                "Default",
            ];
            entitlementsReleasePlist["com.apple.developer.applesignin"] = [
                "Default",
            ];
        ...
        if (
            typeof pluginVariables["FIREBASE_FCM_AUTOINIT_ENABLED"] !==
            "undefined"
        ) {
        ...
        if (googlePlistModified)
            fs.writeFileSync(
                path.resolve(iosPlatform.dest),
                plist.build(googlePlist),
            );
        if (appPlistModified)
            fs.writeFileSync(
                path.resolve(iosPlatform.appPlist),
                plist.build(appPlist),
            );
        if (entitlementsPlistsModified) {
            fs.writeFileSync(
                path.resolve(iosPlatform.entitlementsDebugPlist),
                plist.build(entitlementsDebugPlist),
            );
            fs.writeFileSync(
                path.resolve(iosPlatform.entitlementsReleasePlist),
                plist.build(entitlementsReleasePlist),
            );
        }
    },
    applyPluginVarsToPodfile: function (pluginVariables, iosPlatform) {
        ...
        if (pluginVariables["IOS_FIREBASE_IN_APP_MESSAGING_VERSION"]) {
            if (
                pluginVariables["IOS_FIREBASE_IN_APP_MESSAGING_VERSION"].match(
                    versionRegex,
                )
            ) {
                ...
                if (matches) {
                    ...
                    if (podFileModified)
                        utilities.log(
                            "Firebase iOS InAppMessaging SDK version set to v" +
                                pluginVariables[
                                    "IOS_FIREBASE_IN_APP_MESSAGING_VERSION"
                                ] +
                                " in Podfile",
                ...
                throw new Error(
                    'The value "' +
                        ...
                        '" for IOS_FIREBASE_IN_APP_MESSAGING_VERSION is not a valid semantic version format',
                );
            }
        }
        ...
        if (pluginVariables["IOS_FIREBASE_SDK_VERSION"]) {
            if (
                pluginVariables["IOS_FIREBASE_SDK_VERSION"].match(versionRegex)
            ) {
                var matches = podFileContents.match(firebasePodRegex);
                if (matches) {
                    matches.forEach((match) => {
                        if (
                            match.match(inAppMessagingPodRegEx) &&
                            specifiedInAppMessagingVersion
                        ) {
                ...
                var prebuiltFirestoreMatches = podFileContents.match(
                    prebuiltFirestorePodRegEx,
                );
                ...
                if (podFileModified)
                    utilities.log(
                        "Firebase iOS SDK version set to v" +
                            pluginVariables["IOS_FIREBASE_SDK_VERSION"] +
                            " in Podfile",
                    );
            } else {
                throw new Error(
                    'The value "' +
                        pluginVariables["IOS_FIREBASE_SDK_VERSION"] +
                        '" for IOS_FIREBASE_SDK_VERSION is not a valid semantic version format',
                );
            }
        }
        ...
        if (pluginVariables["IOS_GOOGLE_SIGIN_VERSION"]) {
            if (
                pluginVariables["IOS_GOOGLE_SIGIN_VERSION"].match(versionRegex)
            ) {
                ...
                if (matches) {
                    matches.forEach((match) => {
                        ...
                        if (
                            !match.match(
                                pluginVariables["IOS_GOOGLE_SIGIN_VERSION"],
                            )
                        ) {
                            podFileContents = podFileContents.replace(
                                match,
                    ...
                    if (podFileModified)
                        utilities.log(
                            "Google Sign In version set to v" +
                                pluginVariables["IOS_GOOGLE_SIGIN_VERSION"] +
                                " in Podfile",
                        );
                }
                ...
                throw new Error(
                    'The value "' +
                        pluginVariables["IOS_GOOGLE_SIGIN_VERSION"] +
                        '" for IOS_GOOGLE_SIGIN_VERSION is not a valid semantic version format',
                );
            }
        }
        ...
        if (pluginVariables["IOS_GOOGLE_TAG_MANAGER_VERSION"]) {
            if (
                pluginVariables["IOS_GOOGLE_TAG_MANAGER_VERSION"].match(
                    versionRegex,
                )
            ) {
                ...
                if (matches) {
                    matches.forEach((match) => {
                        ...
                        if (
                            !match.match(
                                pluginVariables[
                                    "IOS_GOOGLE_TAG_MANAGER_VERSION"
                                ],
                            )
                        ) {
                            podFileContents = podFileContents.replace(
                    ...
                    if (podFileModified)
                        utilities.log(
                            "Google Tag Manager version set to v" +
                                pluginVariables[
                                    "IOS_GOOGLE_TAG_MANAGER_VERSION"
                                ] +
                                " in Podfile",
                ...
                throw new Error(
                    'The value "' +
                        pluginVariables["IOS_GOOGLE_TAG_MANAGER_VERSION"] +
                        '" for IOS_GOOGLE_TAG_MANAGER_VERSION is not a valid semantic version format',
                );
            }
        }
        ...
        if (pluginVariables["IOS_USE_PRECOMPILED_FIRESTORE_POD"] === "true") {
            if (process.env.SKIP_FIREBASE_FIRESTORE_SWIFT) {
                var standardFirestorePodMatches = podFileContents.match(
                    standardFirestorePodRegEx,
                );
                if (standardFirestorePodMatches) {
                    podFileContents = podFileContents.replace(
                        ...
                        prebuiltFirestorePodTemplate.replace(
                            "{version}",
                            standardFirestorePodMatches[1],
                        ),
                    );
                    podFileModified = true;
                    utilities.log(
                        "Configured Podfile for pre-built Firestore pod",
                    );
                }
            } else {
                throw new Error(
                    "The environment variable SKIP_FIREBASE_FIRESTORE_SWIFT is not set. This is required to use the pre-built Firestore pod.",
                );
            }
        }
        if (podFileModified) {
            fs.writeFileSync(
                path.resolve(iosPlatform.podFile),
                podFileContents,
            );
        }
